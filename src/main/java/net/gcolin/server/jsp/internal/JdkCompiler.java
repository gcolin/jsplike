...
public class JdkCompiler implements Compiler {
  ...
  @Override
  public ClassLoader compile(String[] targetClassName, String[] source, ClassLoader classLoader,
      File work, boolean writeClasses) throws IOException {
    ...
    for (int i = 0; i < targetClassName.length; i++) {
      ...
      try (Writer out =
          Io.writer(sourceFiles[i].openOutputStream(), StandardCharsets.UTF_8.name())) {
        out.write(source[i]);
      }
    }
    ...
    MutableClassLoader cl =
        AccessController.doPrivileged(new PrivilegedAction<MutableClassLoader>() {
    ...
    JavaCompiler.CompilationTask task = javac.getTask(sw, fileManager, dlistener,
        Arrays.asList("-encoding", "UTF-8", "-proc:none"), null, compilationUnits);
    ...
    if (!success) {
      ...
      for (Diagnostic<? extends JavaFileObject> diagnostic : dlistener.getDiagnostics()) {
        ...
        str.append(diagnostic.getSource() == null ? "" : diagnostic.getSource().getName())
