...
import net.gcolin.server.jsp.internal.exp.ArrayJExpression;
...
public class LeftBracketOperator extends Operator {
  ...
  public JExpression build(JExpression[] args, BuildContext context) {
    JExpression ref = args[args.length - 1];
    JExpression index = args[args.length - 2];
    if (ref.getType().isArray()) {
      return new ArrayJExpression(ref, index);
    } else {
      return new MethodOperator("get", context).build(args, context);
    }
  }
